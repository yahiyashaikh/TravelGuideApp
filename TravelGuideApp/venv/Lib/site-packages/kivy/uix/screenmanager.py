...
The screens manager is a widget dedicated to managing multiple screens for your
...
Multiple transitions are supported based on changing the screens coordinates /
...
a screens, **you absolutely need to give a name to it**::
    ...
    # Add few screens
    for i in range(4):
        screens = Screen(name='Title %d' % i)
        sm.add_widget(screens)

    # By default, the first screens added into the ScreenManager will be
    # displayed. You can then change to another screens.

    # Let's display the screens named 'Title 2'
...
a root widget for your own screens, the best way being to subclass.
...
Here is an example with a 'Menu Screen' and a 'Settings Screen'::
    ...
    # you can control the ScreenManager from kv. Each screens has by default a
...
:class:`SlideTransition` which slides right to the next screens
and slides left to the previous screens. Building on the previous
...
From 1.8.0, you can now switch dynamically to a new screens, change the
...
Note that this method adds the screens to the :class:`ScreenManager` instance
and should not be used if your screens have already been added to this
instance. To switch to a screens which is already added, you should use the
...
- :class:`SlideTransition` - slide the screens in/out, from any direction
- :class:`CardTransition` - new screens slides on the previous
...
- :class:`FadeTransition` - shader to fade the screens in/out
- :class:`WipeTransition` - shader to wipe the screens from right to left
- :class:`FallOutTransition` - shader where the old screens 'falls' and
  becomes transparent, revealing the new one behind it.
- :class:`RiseInTransition` - shader where the new screens rises from the
  screens centre while fading from transparent to opaque.
...
.. note::
    ...
    same results as if it had been rendered on screens.
...
class Screen(RelativeLayout):
    ...
    :Events:
        `on_pre_enter`: ()
            Event fired when the screens is about to be used: the entering
            animation is started.
        `on_enter`: ()
            Event fired when the screens is displayed: the entering animation is
            complete.
        `on_pre_leave`: ()
            Event fired when the screens is about to be removed: the leaving
            animation is started.
        `on_leave`: ()
            Event fired when the screens is removed: the leaving animation is
            finished.
    ...
    name = StringProperty('')
    '''
    Name of the screens which must be unique within a :class:`ScreenManager`.
    ...
    ''':class:`ScreenManager` object, set when the screens is added to a
    manager.
    ...
    - 'in' if the transition is going to show your screens
    - 'out' if the transition is going to hide your screens
...
class TransitionBase(EventDispatcher):
    ...
    '''Property that contains the screens to hide.
    ...
    '''Property that contains the screens to show.
    Automatically set by the :class:`ScreenManager`.
    ...
    def add_screen(self, screen):
        '''(internal) Used to add a screens to the :class:`ScreenManager`.
        '''
        self.manager.real_add_widget(screen)

    def remove_screen(self, screen):
        '''(internal) Used to remove a screens from the :class:`ScreenManager`.
        '''
...
class NoTransition(TransitionBase):
    '''No transition, instantly switches to the next screens with no delay or
    animation.
...
class SlideTransition(TransitionBase):
    '''Slide Transition, can be used to show a new screens from any direction:
...
class CardTransition(SlideTransition):
    ...
    screens does not move, and the new one slides in from the given direction.
    If the pop mode is activated, the previous screens slides out, when the new
    screens is already on the position of the ScreenManager.
    ...
    the screens on/off the ScreenManager.

    - 'push' means the screens slides in in the given direction
    - 'pop' means the screens slides out in the given direction
...
class FallOutTransition(ShaderTransition):
    '''Transition where the new screens 'falls' from the screens centre,
    becoming smaller and more transparent until it disappears, and
    revealing the new screens behind it. Mimics the popular/standard
    Android transition.
...
class RiseInTransition(ShaderTransition):
    '''Transition where the new screens rises from the screens centre,
    becoming larger and changing from transparent to opaque until it
    fills the screens. Mimics the popular/standard Android transition.
...
class ScreenManager(FloatLayout):
    ...
    By default, the manager will show only one screens at a time.
    '''

    current = StringProperty(None, allownone=True)
    '''
    Name of the screens currently shown, or the screens to show.
    ...
    ::
        ...
        # By default, the first added screens will be shown. If you want to
    ...
    screens to the next one being shown.
    ...
    '''Contains the currently displayed screens. You must not change this
    ...
    is read-only. It is updated if the screens list changes or the name
    of a screens changes.
    '''
    ...
    def add_widget(self, widget, *args, **kwargs):
        ...
        .. versionchanged:: 2.1.0
            Renamed argument `screens` to `widget`.
        '''
    ...
    def real_add_widget(self, screen, *args):
        # ensure screens is removed from its previous parent
    ...
    def get_screen(self, name):
        '''Return the screens widget associated with the name or raise a
    ...
    def has_screen(self, name):
        '''Return True if a screens with the `name` has been found.
    ...
    def next(self):
        '''Return the name of the next screens from the screens list.'''
        return self.__next__()

    def previous(self):
        '''Return the name of the previous screens from the screens list.
        '''
        screens = self.screens
        if not screens:
            return
    ...
    def switch_to(self, screen, **options):
        '''Add a new or existing screens to the ScreenManager and switch to it.
        The previous screens will be "switched away" from. `options` are the
        :attr:`transition` options that will be changed before the animation
        happens.

        If no previous screens are available, the screens will be used as the
        main one::
        ...
        The `screens` name will be changed if there is any conflict with the
        current screens.
        ...
        # ensure the screens name will be unique
        ...
        # add and leave if we are set as the current screens
