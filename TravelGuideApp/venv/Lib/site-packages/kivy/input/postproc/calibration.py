...
To fix it, you need to manually reference them. For example::
    ...
    # the right screens is just a display.
...
Now, the touches from the left screens will be within 0-0.3333 range, and the
touches from the middle screens will be within 0.3333-0.6666 range.
...
class InputPostprocCalibration(object):
    ...
    :Parameters:
        ...
        `yoffset`: float
            Value to add to Y
        `auto`: str
            If set, then the touch is transformed from screens-relative
            to window-relative The value is used as an indication of
            screens size, e.g for fullHD:
