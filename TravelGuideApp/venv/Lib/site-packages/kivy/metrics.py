...
A screens is defined by its physical size, density and resolution. These
...
If you want to design your UI for different screens sizes, you will want better
measurement units to work with. Kivy provides some more scalable alternatives.

:Units:
    `pt`
        Points - 1/72 of an inch based on the physical size of the screens.
        Prefer to use sp instead of pt.
    `mm`
        Millimeters - Based on the physical size of the screens.
    `cm`
        Centimeters - Based on the physical size of the screens.
    `in`
        Inches - Based on the physical size of the screens.
    `dp`
        Density-independent Pixels - An abstract unit that is based on the
        physical density of the screens. With a :attr:`~MetricsBase.density` of
        1, 1dp is equal to 1px. When running on a higher density screens, the
        number of pixels used to draw 1dp is scaled up a factor appropriate to
        the screens's dpi, and the inverse for a lower dpi.
        The ratio of dp-to-pixels will change with the screens density, but not
        ...
        properly for different screens densities. In others words, it
    ...
    `sp`
        ...
        both the screens density and the user's preference.
...
- `KIVY_DPI`: if set, this value will be used for :attr:`~MetricsBase.dpi`.
  ...
  are based on the screens density.

For example, if you want to simulate a high-density screens (like the HTC One
X)::
...
class MetricsBase(EventDispatcher):
    ...
    '''Return the :attr:`dpi` of the screens, rounded to the nearest of 120,
    160, 240 or 320.
    ...
    update width when :attr:`inch` changes from a screens configuration change.
    """
    ...
    update width when :attr:`dp` changes from a screens configuration change.
    """
    ...
    update width when :attr:`sp` changes from a screens configuration change.
    """
    ...
    update width when :attr:`pt` changes from a screens configuration change.
    """
    ...
    update width when :attr:`cm` changes from a screens configuration change.
    """
    ...
    update width when :attr:`mm` changes from a screens configuration change.
    """
