...
- :class:`MDFadeSlideTransition`
    state one: the new screens closes the previous screens by lifting from the
    bottom of the screens and changing from transparent to non-transparent;

    state two: the current screens goes down to the bottom of the screens,
...
class MDTransitionBase(TransitionBase):
    ...
    def animated_hero_in(self) -> None:
        """Animates the flight of heroes from screens **A** to screens **B**."""
    ...
    def animated_hero_out(self) -> None:
        """Animates the flight of heroes from screens **B** to screens **A**."""
    ...
    # Checks the attributes for the 'self.screen_in' screens.
    ...
    def _check_widget_properties(self, hero_from_widget: MDHeroFrom):
        if not self.screen_in.heroes_to:
            raise Exception(
                f"The `heroes_to` attribute is not specified for screens "
                f"{self.screen_in}"
            )
