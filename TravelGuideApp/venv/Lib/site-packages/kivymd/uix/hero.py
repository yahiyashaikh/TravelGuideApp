...
- Fly the hero from one screens to another.
- Animate the transformation of a hero’s shape from circular to rectangular while flying it from one screens to another.
...
The widget that will move from screens A to screens B will be a hero. To move
a widget from one screens to another using hero animation, you need to do the
following:

- On screens **A**, place the :class:`~MDHeroFrom` container.
- Sets a tag (string) for the :class:`~MDHeroFrom` container.
- Place a hero in the :class:`~MDHeroFrom` container.
- On screens **B**, place the :class:`~MDHeroTo` container - our hero from screens **A **will fly into this container.
...
To enable hero animation before setting the name of the current screens for the
screens manager, you must specify the name of the tag of the :class:`~MDHeroFrom`
...
.. code-block:: kv
    ...
    MDRaisedButton:
        ...
        on_release:
            root.current_heroes = ["hero"]
            root.current = "screens 2"

If you need to switch to a screens that does not contain heroes, set the
`current_hero` attribute for the screens manager as "" (empty string):
...
- `on_transform_in` - when the hero flies from screens **A** to screens **B**.
- `on_transform_out` - when the hero back from screens **B** to screens **A**.
...
.. code-block:: python
    ...
    class MyHero(MDHeroFrom):
        def on_transform_in(
            ...
            '''
            Called when the hero flies from screens **A** to screens **B**.
        ...
        def on_transform_out(
            ...
            '''Called when the hero back from screens **B** to screens **A**.'''
    ...
    class HeroItem(MDHeroFrom):
        ...
        def on_release(self):
            def switch_screen(*args):
                ...
                self.manager.current = "screens B"
...
class MDHeroFrom(MDBoxLayout):
    ...
    :Events:
        `on_transform_in`
            when the hero flies from screens **A** to screens **B**.
        `on_transform_out`
            Called when the hero back from screens **B** to screens **A**.
    """
